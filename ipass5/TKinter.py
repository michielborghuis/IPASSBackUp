...
class GUI:
    def __init__(self, choice):
        ...
        self.country_list = ipass5.populations.list_of_countries()
        ...
        self.year_list = ipass5.populations.list_of_years()
        ...
        if self.choice == 0:
            self.inPopulation.destroy()

        if self.choice == 1:
            self.dropCountry.destroy()
            self.dropYear.destroy()
    ...
    def labels(self):  # Creating a label widgets
        if self.choice == 0:
    ...
    def calculate(self):
        ...
        if self.check() == 0:
            ...
            print(method)
            if self.choice == 0:
                model = Derivatives([self.clickedCountry.get(), self.clickedYear.get()], float(self.inInitialExposed.get()),
                ...
                model = Derivatives(float(self.inPopulation.get()), float(self.inInitialExposed.get()),
                                    float(self.inSpreadDays.get()), float(self.inIncubation.get()), float(self.inR0.get()),
                                    float(self.inDeathRate.get()), float(self.inDeathTime.get()),
                                    float(self.inLockdown.get()),
                                    float(self.inLockdownR0.get()), method)
    ...
    def check(self):
        ...
        try:
            ...
            if float(self.inLockdownR0.get()) < 0:
                self.error_label_lockdown_r0.grid(
                    row=108, columnspan=4, sticky='w')
                error_count += 1
            else:
